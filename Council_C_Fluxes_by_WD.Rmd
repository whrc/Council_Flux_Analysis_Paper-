...
#needs continuous day variable - daily avg, updated seasons, SWC-nf, winter-year
...
#Add WD_cardinal to df if not already added (which has the half-hourly data)
```{r}
...
#Save df with cardinal wind directions 
```{r}
#Save df with cardinal WD (optional)
write.csv(x = df2,file = 'C:/Users/kkent/Documents/Council Data/Council BASE gapfilling/council_gapfilled_clean_2017_2023_for analysis.3.csv',quote = F,row.names = F)
```
...
#df_avg
df_avg$WD_cardinal2 <- factor(df_avg$WD_cardinal2, 
                                      levels = c("N", "NE", "E", "SE", "S", "SW", "W", "NW"))
...
df_avg_FC <- df_avg %>% 
  filter(winter_year != 2016, winter_year != 2023) %>% #remove years with incomplete seasons; only use years with full seasons   
  filter(complete.cases(FC, season, WD))

df_avg_FCH4 <- df_avg %>% 
    filter(winter_year != 2016, winter_year != 2023) %>% #remove years with incomplete seasons  
  filter(complete.cases(FCH4, season, WD))
...
HH_wind_FCH4 <- df2 %>%
  ...
  dplyr::filter(complete.cases(season, WD, FCH4)) %>%
  mutate(WD_cardinal = factor(WD_cardinal, 
                              levels = c("N", "NE", "E", "SE", "S", "SW", "W", "NW")))


ggplot (HH_wind_FCH4, aes(x = date, y = FCH4, color = WD_cardinal )) +
  geom_point() +
...
ggplot(HH_wind_FCH4, #%>%
     # filter(season == "Growing Season"), 
      aes(x = date, y = FCH4 * 1/1000, color = WD_cardinal)) +
  geom_point() + 
   geom_hline(yintercept = 0)+
 #scale_x_date(date_breaks = "1 year", date_labels = "%Y")+ #forced labels for each year 
  #scale_x_date(date_breaks = "6 months", date_labels = "%b %Y")+ #month and year - too crowded 
  scale_x_date(breaks = as.Date(c("2017-07-14", "2018-01-01", "2019-01-01", 
  ...
  legend.title = element_text(face = "bold", size = 14))+
    labs(
    x = expression(bold("Date")),
    y = expression(bold("Half-hourly CH"[4] * " Flux ("*mu*mol/m^2/s*")")),
    color = "Wind Direction",
    
    title = expression(bold("CH"[4] ~ "by Wind Direction ")))
...
HH_wind_FC<- df2 %>%
 # filter(winter_year != "2016", winter_year != "2023") %>%
  dplyr::filter(complete.cases(season, WD, FC)) %>%
    mutate(WD_cardinal = factor(WD_cardinal, 
                              levels = c("N", "NE", "E", "SE", "S", "SW", "W", "NW")))

ggplot (HH_wind_FC, aes(x = date, y = FC, color =WD_cardinal )) +
  geom_point() +
  facet_wrap(~season, ncol = 3)
...
ggplot(HH_wind_FC, #%>%
     # filter(season == "Growing Season"), 
      aes(x = date, y = FC, color = WD_cardinal)) +
  geom_point() + 
   geom_hline(yintercept = 0)+
 #scale_x_date(date_breaks = "1 year", date_labels = "%Y")+ #forced labels for each year 
  #scale_x_date(date_breaks = "6 months", date_labels = "%b %Y")+ #month and year - too crowded 
  ...
  legend.title = element_text(face = "bold", size = 14))+
    labs(
    x = expression(bold("Date")),
    y = expression(bold("Half-hourly CO"[2] * " Flux ("*mu*mol/m^2/s*")")),
    color = "Wind Direction",
    title = expression(bold("CO"[2] ~ "by Wind Direction ")))

```

#df_avgflux figure by WD 

### FCH4 by WD - daily avg

```{r}

avg_wind_FCH4 <- df_avg_FCH4 %>%
    #filter(winter_year != "2016", winter_year != "2023") %>%
  dplyr::filter(complete.cases(season, WD, FCH4)) %>%
  mutate(WD_cardinal = factor(WD_cardinal, 
                              levels = c("N", "NE", "E", "SE", "S", "SW", "W", "NW")))
...
ggplot (avg_wind_FCH4, aes(x = date, y = FCH4, color = WD_cardinal )) +
  geom_point() +
  facet_wrap(~season, ncol = 3)


p_avg_wind_FCH4 <- ggplot(avg_wind_FCH4, #%>%
     # filter(season == "Growing Season"), 
      aes(x = date, y = FCH4 * 1/1000, color = WD_cardinal)) +
  geom_point() + 
   geom_hline(yintercept = 0)+
 #scale_x_date(date_breaks = "1 year", date_labels = "%Y")+ #forced labels for each year 
  #scale_x_date(date_breaks = "6 months", date_labels = "%b %Y")+ #month and year - too crowded 
  scale_x_date(breaks = as.Date(c("2017-07-19", "2018-01-01", "2019-01-01", 
                                   "2020-01-01", "2021-01-01", "2022-01-01", "2023-01-01")),
               date_labels = "%Y") +
  ...
  legend.title = element_text(face = "bold", size = 14))+
    labs(
    x = expression(bold("Date")),
    y = expression(bold(atop("Daily Average CH"[4] * " Flux", "("*mu*mol/m^2/s*")"))),
    color = "Wind Direction",
...
#save figure 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/avg_FCH4_WD_timeseries.png", p_avg_wind_FCH4, 
...
ggplot (avg_wind_FC, aes(x = date, y = FC, color = WD_cardinal )) +
  geom_point() +
  facet_wrap(~season, ncol = 3)


p_avg_wind_FC <- ggplot(avg_wind_FC, #%>%
     # filter(season == "Growing Season"), 
      aes(x = date, y = FC, color = WD_cardinal)) +
  geom_point() + 
   geom_hline(yintercept = 0)+
 #scale_x_date(date_breaks = "1 year", date_labels = "%Y")+ #forced labels for each year 
  #scale_x_date(date_breaks = "6 months", date_labels = "%b %Y")+ #month and year - too crowded 
  scale_x_date(breaks = as.Date(c("2017-07-17", "2018-01-01", "2019-01-01", 
                                   "2020-01-01", "2021-01-01", "2022-01-01", "2023-01-01")),
               date_labels = "%Y") +
  ...
  legend.title = element_text(face = "bold", size = 14))+
    labs(
    x = expression(bold("Date")),
    y = expression(bold(atop("Daily Average CO"[2] * "Flux","("*mu*mol/m^2/s*")"))),
    color = "Wind Direction",
...
#save figure 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/avg_FC_WD_timeseries.png", p_avg_wind_FC, 
       width = 8, height = 5, dpi = 600)
```



#Combine the two figs 
```{r}
library(patchwork)
# Remove legend from first plot, keep it on second, then combine

# # Create modified plots first
# p1 <- p_avg_wind_FC + theme(legend.position = "none")
# p2 <- p_avg_wind_FCH4
# 
# # Then combine
# p1 / p2 + 
#   plot_layout(guides = "collect") +
#   plot_annotation(title = "Carbon Fluxes by Wind Direction") &
#   theme(legend.position = "right")

# Stack vertically with shared legend on right
p_C_WD_combined <- (p_avg_wind_FC + theme(legend.position = "none")) /
  p_avg_wind_FCH4 + 
  plot_layout(guides = "collect") +
 # plot_annotation(title = "Carbon Fluxes by Wind Direction") &
  theme(legend.position = "right")
...
#save figure 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/avg_C_WD_timeseries_combined.png", p_C_WD_combined, 
...
# [1] "FCH4 observations: 580"
# [1] "FC observations: 821"
# [1] "\nFCH4 observations by season and wind direction:"
#                  
#                     N  NE   E  SE   S  SW   W  NW
#   Fall Senescence  15   4   1   4   2   4   0  48
#   Growing Season   59  11   0  29  74  17   1 149
#   Winter           29  14   0  15   2   2   2  98
# [1] "\nFC observations by season and wind direction:"
#                  
#                     N  NE   E  SE   S  SW   W  NW
#   Fall Senescence  24   4   8  16   7   4   2  90
#   Growing Season   74  12   1  60 130  25   1 184
#   Winter           25  15   1  12   6   2   3 115
...
create_flux_summary <- function(fch4_data, fc_data, min_n = 5) {
  ...
  fch4_summary <- fch4_data %>%
    ...
    summarise(
      n_obs_FCH4 = n(),
      FCH4_mean = round(mean(FCH4 * (1/1000), na.rm = TRUE), 3), #nmol to umol
      FCH4_se = round(sd(FCH4 * (1/1000), na.rm = TRUE) / sqrt(n()), 3), #nmol to umol 
      .groups = 'drop'
    ) %>%
...
#note: FC is in umol/m2/s; FCH4 is in nmol/m2/s -> but converted to umol by FCH4 * (1/1000)
```
...
kruskal.test(FC ~ WD_cardinal2, data = test) #p = 0.00639
...
#checking winter pairs for letter assignment 

test <- fc_winter %>%
  filter(!WD_cardinal2 == "E" & !WD_cardinal2 == "W" & !WD_cardinal2 == "SW") #removing the WD with n<5


kruskal.test(FC ~ WD_cardinal2, data = test) #p = < 0.001


dunn_result <- dunnTest(FC ~ WD_cardinal2, 
                       data = subset(test, season == "Winter"),
                       method = "bh")
dunn_result
...
fc_letter_positions_updated <- df_avg_FC_with_n %>%
  ...
  filter(!is.na(letters)) %>%
  mutate(
    letter_y = 2.5 #puts letters on same y-coord 
      #pmax(max_y, q75) + 0.7  # Using your preferred spacing - puts letters just above box
  )
...
FC_by_WD_final_updated <- ggplot(df_avg_FC_with_n, aes(x = WD_cardinal2, y = FC, fill = season)) +
  ...
  expand_limits(y = max(df_avg_FC_with_n$FC, na.rm = TRUE) * 1.2) +
...
#save figure with statistical lettering 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/adjFC_by_WD_seasons_stats2.png", FC_by_WD_final_updated, 
       width = 10, height = 7, dpi = 600)
```
...
#save figure with statistical lettering 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/adjFCH4_by_WD_seasons_stats2.png", FCH4_by_WD_final, 
       width = 10, height = 7, dpi = 600)
```

#pairwise Dunn test to double check stats letter assignment 
```{r}
#checking winter pairs for letter assignment 
...
kruskal.test(FCH4 ~ WD_cardinal2, data = test) #p = < 0.001
...
dunn_result_table <- dunn_result$res %>%
  mutate(
    P.unadj = format(P.unadj, scientific = FALSE, digits = 4),
    P.adj   = format(P.adj, scientific = FALSE, digits = 4)
  )
...
#Binning wind by landscape features at Council 

SW, S, SW, = drainage channels / thermokarst
N, NE = ponds and forested/riparian area 
W = open tundra with some tree patches, also looks like drainage or thermokarsting areas 

```{r}
library(dplyr)

 #create dataset with variables of interest - these datasets already have cc for FC and FCH4 
landscape_FC <- df_avg_FC %>%
  select(date, year, season, FC, GPP, RECO, WD_modal2, WD_cardinal2)


landscape_FCH4 <- df_avg_FCH4 %>%
  select(date, year, season, FCH4, WD_modal2, WD_cardinal2)


#check
sum(is.na(landscape_FC$FC))
sum(is.na(landscape_FCH4$FCH4))
```

#### Assign landscape features to WD_cardinal2
```{r}

# Assign landscape feature based on cardinal directions & map of council tower location 

#FC
  landscape_FC <- landscape_FC %>%
    mutate(
    landscape_feature = case_when(
      WD_cardinal2 == "N" | WD_cardinal2 == "NE" | WD_cardinal2 == "NW" ~ "Ponds, Riparian (N, NE, NW)", 
      WD_cardinal2 == "S" | WD_cardinal2 == "SE" ~ "Drainage Channels (S, SE)", 
      WD_cardinal2 == "W" | WD_cardinal2 == "SW" ~ "Tundra, Wet Lowlands (W, SW)", 
      WD_cardinal2 == "E"  ~ "Tundra, Road (E)", 
...
ggplot(landscape_FC, aes (x = landscape_feature, y = FC, fill = landscape_feature ))+
  geom_boxplot()+
  stat_summary(fun = mean, geom = "point", shape = 23, size = 4) +
  facet_wrap(~season) +
  theme_bw()
...
#nmol to umol 
ggplot(landscape_FCH4, aes (x = landscape_feature, y = FCH4 * 1/1000, fill = landscape_feature ))+
  geom_boxplot()+
  stat_summary(fun = mean, geom = "point", shape = 23, size = 4) +
  facet_wrap(~season) +
  theme_bw()+
scale_fill_manual(name = "Landscape Feature",
            values = c("Drainage Channels (S, SE)" = "cyan3", 
                      "Ponds, Riparian (N, NE, NW)" = "royalblue", 
                      "Tundra, Road (E)" = "brown", 
                      "Tundra, Wet Lowlands (W, SW)" = "chartreuse4")
  ) +
  theme(
  text = element_text(face = "bold") #makes ALL text within fig bold 
)



#levels(factor(landscape_FCH4$landscape_feature))

```
#Create factors for analysis
```{r}
#FC
df_analysis_FC.2 <- landscape_FC %>%
  mutate(
    # Create factors for analysis
    landscape_feature = factor(landscape_feature, levels = c("Ponds, Riparian (N, NE, NW)", "Tundra, Road (E)", "Drainage Channels (S, SE)","Tundra, Wet Lowlands (W, SW)")),
    season = factor(season, levels = c("Winter", "Growing Season", "Fall Senescence"))
  )

#FCH4
df_analysis_FCH4.2 <- landscape_FCH4 %>%
  mutate(
    # Create factors for analysis
    landscape_feature = factor(landscape_feature, levels = c("Ponds, Riparian (N, NE, NW)", "Tundra, Road (E)", "Drainage Channels (S, SE)","Tundra, Wet Lowlands (W, SW)")),
    season = factor(season, levels = c("Winter", "Growing Season", "Fall Senescence"))
...
#Stats on C by WD from landscape feature 

note: FC is in umol/m2/s; FCH4 is in nmol/m2/s

#Streamlined analyses code from Claude - non-parametric with BH corrections, filtered at n >= 5

```{r}
# FLUX ANALYSIS BY LANDSCAPE FEATURE
# Tests if FCH4 and FC differ by landscape feature within each season
...
df_analysis_FC.2 <- landscape_FC %>%
  mutate(
    landscape_feature = factor(landscape_feature, 
                               levels = c("Ponds, Riparian (N, NE, NW)", 
                                        "Tundra, Road (E)", 
                                        "Drainage Channels (S, SE)",
                                        "Tundra, Wet Lowlands (W, SW)")),
    season = factor(season, levels = c("Winter", "Growing Season", "Fall Senescence"))
  )
...
create_flux_summary <- function(fch4_data, fc_data, min_n = 5) {
  ...
  # FCH4 summary
  fch4_summary <- fch4_data %>%
    group_by(season, landscape_feature) %>%
    summarise(
      n_obs_FCH4 = n(),
  ...
  # Merge summaries
  combined_summary <- full_join(fch4_summary, fc_summary, 
                                by = c("season", "landscape_feature")) %>%
    mutate(
      n_obs_FCH4 = coalesce(n_obs_FCH4, 0),
      n_obs_FC = coalesce(n_obs_FC, 0),
      adequate_n_FCH4 = ifelse(n_obs_FCH4 >= min_n, "Yes", "No"),
    ...
    arrange(season, landscape_feature)
  
  return(combined_summary)
}

# Create summary table
summary_table <- create_flux_summary(df_analysis_FCH4.2, df_analysis_FC.2, min_n = 5)

cat("\n", rep("=", 80), "\n")
cat("SUMMARY STATISTICS BY SEASON AND LANDSCAPE FEATURE\n")
cat("Minimum n = 5 for statistical analysis\n")
cat(rep("=", 80), "\n")
...
for(season_name in c("Winter", "Growing Season", "Fall Senescence")) {
  ...
  season_data <- summary_table %>% filter(season == season_name)
  
  if(nrow(season_data) > 0) {
    # FCH4 table
    cat("** FCH4 (nmol/m²/s) **\n")
    cat(sprintf("%-35s %6s %12s %10s\n", 
                "Landscape Feature", "n", "Mean±SE", "Adequate"))
    cat(rep("-", 70), "\n")
    
    for(i in 1:nrow(season_data)) {
      if(season_data$n_obs_FCH4[i] > 0) {
        mean_se <- sprintf("%.3f±%.3f", season_data$FCH4_mean[i], 
                          season_data$FCH4_se[i])
      } else {
        mean_se <- "No data"
      }
      cat(sprintf("%-35s %6d %12s %10s\n",
                 as.character(season_data$landscape_feature[i]),
                 season_data$n_obs_FCH4[i],
                 mean_se,
                 season_data$adequate_n_FCH4[i]))
    }
    ...
    for(i in 1:nrow(season_data)) {
      if(season_data$n_obs_FC[i] > 0) {
        mean_se <- sprintf("%.3f±%.3f", season_data$FC_mean[i], 
                          season_data$FC_se[i])
      } else {
        mean_se <- "No data"
      }
      cat(sprintf("%-35s %6d %12s %10s\n",
                 as.character(season_data$landscape_feature[i]),
                 season_data$n_obs_FC[i],
                 mean_se,
                 season_data$adequate_n_FC[i]))
    }
  } else {
...
run_landscape_analysis <- function(data, response_var, season_name, min_n = 5) {
  
  cat("\n", rep("=", 80), "\n")
  cat("ANALYSIS:", toupper(response_var), "- SEASON:", toupper(season_name), "\n")
  cat(rep("=", 80), "\n")
  
  # Check sample sizes by landscape feature
  feature_counts <- table(data$landscape_feature)
  adequate_features <- names(feature_counts[feature_counts >= min_n])
  
  # Find excluded features
  all_possible_features <- levels(data$landscape_feature)
  excluded_features <- all_possible_features[!all_possible_features %in% adequate_features]
  
  cat("\nSample sizes by landscape feature:\n")
  for(feat in all_possible_features) {
    n_obs <- ifelse(feat %in% names(feature_counts), feature_counts[feat], 0)
    status <- ifelse(n_obs >= min_n, "✓ Adequate", "✗ Excluded")
    cat(sprintf("  %-35s: n=%3d  %s\n", feat, n_obs, status))
  }
  
  if(length(adequate_features) < 2) {
    cat("\n*** INSUFFICIENT LANDSCAPE FEATURES FOR ANALYSIS ***\n")
    cat("Need at least 2 features with n ≥", min_n, "\n")
    return(NULL)
  }
  
  # Filter to adequate features only
  filtered_data <- data %>%
    filter(landscape_feature %in% adequate_features) %>%
    mutate(landscape_feature = factor(landscape_feature, levels = adequate_features))
  
  cat("\nFiltered dataset: n =", nrow(filtered_data), "observations\n")
  cat("Analyzing", length(adequate_features), "landscape features\n")
  
  # Kruskal-Wallis test
  if(nrow(filtered_data) >= 10) {
    kw_formula <- as.formula(paste(response_var, "~ landscape_feature"))
    kw_result <- kruskal.test(kw_formula, data = filtered_data)
    ...
    if(kw_result$p.value < 0.05) {
      ...
      cat("Running Dunn's post-hoc test with BH correction...\n\n")
      
      dunn_result <- dunnTest(kw_formula, data = filtered_data, method = "bh")
      
      # Show all comparisons
      cat("** All Pairwise Comparisons **\n")
      print(dunn_result$res[, c("Comparison", "Z", "P.unadj", "P.adj")])
      ...
      return(list(
        kruskal_result = kw_result,
        dunn_result = dunn_result,
        filtered_data = filtered_data,
        adequate_features = adequate_features,
        excluded_features = excluded_features
      ))
      
    } else {
      cat("\nNo significant differences among landscape features\n")
      return(list(
        kruskal_result = kw_result,
        filtered_data = filtered_data,
        adequate_features = adequate_features,
    ...
    cat("\n*** INSUFFICIENT OBSERVATIONS FOR RELIABLE TESTING ***\n")
    cat("Need at least 10 total observations\n")
    return(NULL)
  }
}

# ============================================================================
# 3. RUN ANALYSES FOR ALL COMBINATIONS
# ============================================================================

cat("\n\n", rep("#", 80), "\n")
cat("STATISTICAL ANALYSIS: DIFFERENCES BY LANDSCAPE FEATURE\n")
cat(rep("#", 80), "\n")
...
# FCH4 - Winter
fch4_winter_data <- df_analysis_FCH4.2 %>% filter(season == "Winter")
all_results$FCH4_winter <- run_landscape_analysis(fch4_winter_data, "FCH4", "Winter")

# FCH4 - Growing Season
fch4_growing_data <- df_analysis_FCH4.2 %>% filter(season == "Growing Season")
all_results$FCH4_growing <- run_landscape_analysis(fch4_growing_data, "FCH4", "Growing Season")

# FCH4 - Fall Senescence
fch4_fall_data <- df_analysis_FCH4.2 %>% filter(season == "Fall Senescence")
all_results$FCH4_fall <- run_landscape_analysis(fch4_fall_data, "FCH4", "Fall Senescence")

# FC - Winter
fc_winter_data <- df_analysis_FC.2 %>% filter(season == "Winter")
all_results$FC_winter <- run_landscape_analysis(fc_winter_data, "FC", "Winter")

# FC - Growing Season
fc_growing_data <- df_analysis_FC.2 %>% filter(season == "Growing Season")
all_results$FC_growing <- run_landscape_analysis(fc_growing_data, "FC", "Growing Season")

# FC - Fall Senescence
fc_fall_data <- df_analysis_FC.2 %>% filter(season == "Fall Senescence")
all_results$FC_fall <- run_landscape_analysis(fc_fall_data, "FC", "Fall Senescence")
...
cat("\n\n", rep("#", 80), "\n")
cat("FINAL SUMMARY: SIGNIFICANT RESULTS\n")
cat(rep("#", 80), "\n\n")
...
for(analysis_name in names(all_results)) {
  ...
  if(!is.null(result) && !is.null(result$kruskal_result)) {
    if(result$kruskal_result$p.value < 0.05) {
      significant_found <- TRUE
      
      # Parse analysis name
      parts <- strsplit(analysis_name, "_")[[1]]
      var_name <- parts[1]
      season_name <- paste(parts[-1], collapse = " ")
      
      cat("✓", toupper(var_name), "-", toupper(season_name), "\n")
      cat("  Kruskal-Wallis p =", format.pval(result$kruskal_result$p.value), "\n")
      
      if(!is.null(result$dunn_result)) {
        sig_pairs <- result$dunn_result$res[result$dunn_result$res$P.adj < 0.05, ]
        if(nrow(sig_pairs) > 0) {
          cat("  Significant pairs:", nrow(sig_pairs), "\n")
        }
      }
      cat("\n")
    }
...
if(!significant_found) {
  cat("No significant differences detected in any season/variable combination\n")
}
...
write_xlsx(summary_table, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/C_WD_landscape_summary_table.xlsx")

```



#Figure with transparent boxes for landscape features with insufficient n to be used in analyses
```{r}
# BOXPLOTS BY LANDSCAPE FEATURE - MATCHING WIND DIRECTION FIGURE STYLE
# Features with n < 5 shown as transparent
# Boxplots show median (line) and mean (diamond)
...
# ============================================================================
# PREPARE DATA WITH ALPHA LEVELS AND SHORT LABELS
# ============================================================================

# Create short label mapping
short_labels <- c(
  "Ponds, Riparian (N, NE, NW)" = "Ponds,\nRiparian",
  "Tundra, Road (E)" = "Tundra,\nRoad",
  "Drainage Channels (S, SE)" = "Drainage\nChannels",
  "Tundra, Wet Lowlands (W, SW)" = "Tundra,\nWet Lowlands"
)

# Convert FCH4 from nmol to µmol and add alpha column
df_plot_FCH4 <- df_analysis_FCH4.2 %>%
  mutate(
    FCH4_umol = FCH4 * (1/1000),
    landscape_short = factor(short_labels[as.character(landscape_feature)],
                            levels = short_labels)
  ) %>%
  group_by(season, landscape_feature) %>%
  mutate(
    n_obs = n(),
    alpha_level = ifelse(n_obs >= min_n, 1.0, 0.3)
  ) %>%
  ungroup()

# Add alpha column for FC
df_plot_FC <- df_analysis_FC.2 %>%
  mutate(
    landscape_short = factor(short_labels[as.character(landscape_feature)],
                            levels = short_labels)
  ) %>%
...
p_fch4 <- ggplot(df_plot_FCH4, aes(x = landscape_short, y = FCH4_umol, fill = landscape_feature)) +
  
  # Boxplot with conditional transparency
  geom_boxplot(aes(alpha = alpha_level)) +
  
  # Add mean as diamond - filled with same color as box
  stat_summary(aes(alpha = alpha_level, fill = landscape_feature), 
               fun = mean, geom = "point", 
               shape = 23, size = 4, color = "black", stroke = 0.5) +
  ...
  labs(
    x = "Landscape Feature",
    y = expression("Daily Average Half-hourly CH"[4] * " Flux (" * mu * mol ~ m^-2 ~ s^-1 * ")"),
    title = expression("Daily Average Half-hourly CH"[4] ~ "Flux by Landscape Feature"),
    caption = "Reduced opacity: n < 5"
  )
...
p_fc <- ggplot(df_plot_FC, aes(x = landscape_short, y = FC, fill = landscape_feature)) +
  ...
  labs(
    x = "Landscape Feature",
    y = expression("Daily Average Half-hourly CO"[2] * " Flux (" * mu * mol ~ m^-2 ~ s^-1 * ")"),
    title = expression("Daily Average Half-hourly CO"[2] ~ "Flux by Landscape Feature"),
    caption = "Reduced opacity: n < 5"
  )
...
cat("  - Transparent boxes indicate n < 5 (excluded from analysis)\n")
cat("  - Diamond = mean, Box line = median\n")
cat("  - Faceted by season (3 panels)\n")
```
#Adding letters to show statistical differences 
...
# ============================================================================
# PREPARE COMPACT LETTER DISPLAY DATA
# ============================================================================

# For significant comparisons, assign letters
# Groups sharing letters are NOT significantly different
# Groups with different letters ARE significantly different

# FCH4 Growing Season: Drainage vs Ponds significant
fch4_growing_letters <- data.frame(
  season = "Growing Season",
  landscape_short = factor(c("Ponds,\nRiparian", "Tundra,\nRoad", 
                             "Drainage\nChannels", "Tundra,\nWet \nLowlands"),
                          levels = short_labels),
  letters = c("a", "", "b", "b"),  # Drainage (b) differs from Ponds (a), and tundra/wet lowlands (a)
  letter_y = c(0.045, 0.045, 0.045, 0.045)  # Adjust y position as needed
)

# FC Winter: Drainage vs Ponds significant
fc_winter_letters <- data.frame(
  season = "Winter",
  landscape_short = factor(c("Ponds,\nRiparian", "Tundra,\nRoad", 
                             "Drainage\nChannels", "Tundra,\nWet \nLowlands"),
                          levels = short_labels),
  letters = c("a", "ab", "b", "ab"),  # Drainage (b) differs from Ponds (a)
  letter_y = c(2.5, 2.5, 2.5, 2.5)  # Adjust based on data range
)

# FC Growing Season: Drainage vs Ponds significant
fc_growing_letters <- data.frame(
  season = "Growing Season",
  landscape_short = factor(c("Ponds,\nRiparian", "Tundra,\nRoad", 
                             "Drainage\nChannels", "Tundra,\nWet \nLowlands"),
                          levels = short_labels),
...
p_fch4 <- ggplot(df_plot_FCH4, aes(x = landscape_short, y = FCH4_umol, fill = landscape_feature)) +
  ...
  # Add compact letter display for Growing Season only
  geom_text(data = fch4_growing_letters,
            aes(x = landscape_short, y = letter_y, label = letters),
            inherit.aes = FALSE,
            size = 5, fontface = "bold", color = "black") +
  ...
  labs(
    x = "Landscape Feature",
    y = expression("Daily Average Half-hourly CH"[4] * " Flux (" * mu * mol ~ m^-2 ~ s^-1 * ")"),
    title = expression("Daily Average CH"[4] ~ "by Landscape Feature"),
    caption = "Reduced opacity: n < 5. Diamond = mean, line = median. Letters indicate statistical groups for significant seasons (Kruskal-Wallis + Dunn's test, BH correction)."
  )
...
p_fc <- ggplot(df_plot_FC, aes(x = landscape_short, y = FC, fill = landscape_feature)) +
  ...
  # Add compact letter display for Winter and Growing Season
  geom_text(data = rbind(fc_winter_letters, fc_growing_letters),
            aes(x = landscape_short, y = letter_y, label = letters),
            inherit.aes = FALSE,
            size = 5, fontface = "bold", color = "black") +
  ...
  labs(
    x = "Landscape Feature",
    y = expression("Daily Average Half-hourly CO"[2] * " Flux (" * mu * mol ~ m^-2 ~ s^-1 * ")"),
    title = expression("Daily Average CO"[2] ~ "Flux by Landscape Feature"),
    caption = "Reduced opacity: n < 5. Diamond = mean, line = median. Letters indicate statistical groups for significant seasons (Kruskal-Wallis + Dunn's test, BH correction)."
  )
...
#save figure with statistical lettering 
```{r}
ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/FCH4_by_landfeature_stats.png", p_fch4, 
       width = 12, height = 7, dpi = 600)

ggsave( "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/FC_by_landfeature_stats.png", p_fc, 
       width = 12, height = 7, dpi = 600)
```
