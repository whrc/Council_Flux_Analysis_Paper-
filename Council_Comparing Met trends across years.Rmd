...
#Air T + soil T combined into same fig
```{r}
#combined plot: gapfilled air and soil temp 

p_TA.TS <- ggplot(seasonal_summary_all %>%
                 filter(winter_year >= 2017 & winter_year <= 2022)) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +

  #== Air T - circles ===================
  geom_point(aes(x = winter_year, y = TA_gapfilled_mean, color = winteryear_factor, shape = "Gap-filled Air Temperature", group = winter_year), 
             size = 3) +
  geom_errorbar(aes(x = winter_year, ymin = TA_gapfilled_mean - TA_gapfilled_se, 
                    ymax = TA_gapfilled_mean + TA_gapfilled_se, color = "black"), # color = winteryear_factor),
                width = 0.3, linewidth = 0.7, alpha = 1 ) +
...
#=== Soil T - triangles =============================
  geom_point(aes(x = winter_year, y = TS_3_gapfilled_mean, color = winteryear_factor, shape = "Gap-filled Soil Temperature", group = winter_year, pch = 17),
             size = 3) +
  geom_errorbar(aes(x = winter_year, ymin = TS_3_gapfilled_mean - TS_3_gapfilled_se, 
                    ymax = TS_3_gapfilled_mean + TS_3_gapfilled_se, color = "black"), # color = winteryear_factor),
                 width = 0.3, linewidth = 0.7, alpha = 1 ) +
...
facet_wrap(~ season, ncol = 3, scales = "free_x") +  #free_x needed to allow different axis labels among the panels, otherwise they will all be the same for each panel 
  ...
  labs(title = "Mean Seasonal Time Series (2017-2022)",
       x = "Year", 
       y = "Temperature (°C)"
       ) +
  theme_bw() +
...
seasonal_MeanMet_by_winteryr <- grid.arrange(
  arrangeGrob(p_TA.TS, p_SWC_nf, p_VPD, p_RH, ncol = 2), # p_H, p_LE, p_G,
  legend,
  ncol = 2,
  widths = c(10, 1.5),
...
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/annual_Table1_MeanMet_winteryear_plot.png", p_met, 
...
#calculate sens slope for SWC-nf borderline trend
swc_nf_sens.slope <- annual_budgets3 %>%
  summarise(
     # Soil Moisture
    SWC_tau = MannKendall(mean_soil_moisture_nf)$tau,
    SWC_pval = MannKendall(mean_soil_moisture_nf)$sl,
   SWC_slope = ifelse(sum(!is.na(mean_soil_moisture_nf)) >= 3, #make sure enough data, no NAs
                       sens.slope(na.omit(mean_soil_moisture_nf))$estimates,
                       NA))
    
swc_nf_sens.slope #sens slope = -1.7113*


```
#Annual - LM of TA, TS, SWC-nf to compare with Mann-Kendall timeseries 
```{r}
lm_TA <- lm(mean_TA_gapfilled ~ year, data = annual_budgets3)
summary(lm_TA) #R2 = 0.36, slope = -0.46, p = 0.2


lm_TS <- lm(mean_TS_3_gapfilled~ year, data = annual_budgets3)
summary(lm_TS) #R2 = -0.2; slope = 0.03, p = 0.85 

lm_SWC_nf <- lm(mean_soil_moisture_nf ~ year, data = annual_budgets3)
summary(lm_SWC_nf) #R2 = 0.6517, slope = -2.01, p = 0.032** SIG, but for small sample size timeseries, the mann-kendall tends to be more appropriate. will report both. 
...
test_seasonal_trends <- function(data, var_name, var_label, units) {
  ...
  for (s in seasons) {
    ...
    # Test normality (Shapiro-Wilk) - tests residuals 
...
# Mann-Kendall time series stats 
vpd_stats <- data.frame(
  season = c("Growing Season", "Fall Senescence"), #order of labels for panels in facetwrap
  mk_tau = c(0.87, 0.87),  # Replace with your actual adj R² values - Fall, Growing, Winter 
  slope = c(0.90, 0.66),  # Replace with your actual slopes - Fall, Growing, Winter 
  p_value = c(0.02, 0.02),  # Replace with your actual p-values - Fall, Growing, Winter 
  #mann_kendall_p = c(0.027, 0.024),
  #mann_kendall_tau = c(1.00, 0.87),
  x = c(2017, 2017),  # x position for text
  y = c(10, 10)  # y position for text - adjust based on your data range
...
vpd_stats <- vpd_stats %>%
  mutate(season_ordered = factor(season,
    ...
    label = paste0("MK tau = ", round(mk_tau, 2), "\n",
                    "Sens Slope = ", round(slope, 2), "\n",
...
# Add to plot
p_VPD_mk <- ggplot(seasonal_summary_all %>%
  ...
  # Add trend line - all panels 
# geom_smooth(aes(group = season), method = "lm", se = TRUE, 
#               color = "black", linewidth = 0.8, alpha = 0.2) +
  
  # Add trend line ONLY for Growing Season and Fall Senescence - panels with sig trends
  geom_smooth(data = . %>% filter(season %in% c("Growing Season", "Fall Senescence")), #the "data = ." means use the same dataset as indicated in ggplot 
              aes(group = season), 
              method = "lm", se = TRUE, 
              color = "black", linewidth = 0.8, alpha = 0.2) +
...
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/winteryear_VPD_trends_statsMK.png", p_VPD_mk, 
        width = 7, height = 5, dpi = 600)
```
...
#Mann-Kendall timeseries results 
rh_stats <- data.frame(
  season = c("Growing Season", "Fall Senescence", "Winter"),
  mk_tau = c(-1, -1, -1),  # Replace with your actual adj R² values - Fall, Growing, Winter 
  slope = c(-6.06, -7.12, -9.69),  # Replace with your actual slopes - Fall, Growing, Winter 
  p_value = c(0.01, 0.01, 0.01),  # Replace with your actual p-values - Fall, Growing, Winter 
 # mann_kendall_p = c(0.01, 0.01, 0.01),
 # mann_kendall_tau = c(-1.00, -1.00, -1.00),
  x = c(2018, 2018, 2018),  # x position for text
  y = c(100, 100, 100)  # y position for text - adjust based on your data range
...
rh_stats <- rh_stats %>%
  mutate(season_ordered = factor(season,
    ...
    label = paste0(#"adj R² = ", round(r2, 2), "\n",
                       ...
                       # "MK_tau =", round(mann_kendall_tau, 2), "\n",
                       # "p =", format.pval(mann_kendall_p, digits = 4)
                       ))

# Add to plot
p_rh_mk <- ggplot(seasonal_summary_all %>%
                    filter(winter_year != "2016", winter_year != "2023") %>%
                     mutate(season_ordered = factor(season,
                                                    levels = c("Growing Season", "Fall Senescence", "Winter"))),
...
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/winteryear_RH_trends_statsMK.png", p_rh_mk, 
        width = 7, height = 5, dpi = 600) 
```

#SWC-NF borderline sig  in lm, but not sig in MK - using MK results 
```{r}
#Best way to add unique text to each panel in the facet_wrapped figure is to create a dataframe with the info you want to add to each panel 
...
# Remove titles from all plots
p_SWC_nf <- p_SWC_nf + labs(title = NULL) #+ theme(plot.margin = ggplot2::margin(5, 5, 0, 5, "pt")) # top, right, bottom, left
p_VPD_mk <- p_VPD_mk + labs(title = NULL)
p_rh_mk <- p_rh_mk + labs(title = NULL)
p_TA.TS <- p_TA.TS + theme(plot.title = element_blank(), legend.position = "none")
...
# Arrange all your existing plots with the shared legend
seasonal_MeanMet_by_winteryr.MK <- grid.arrange(
  arrangeGrob(p_TA.TS, p_SWC_nf, p_VPD_mk, p_rh_mk, ncol = 2), # p_H, p_LE, p_G,
  legend,
  ncol = 2,
  widths = c(10, 1.5),
  top = textGrob("Seasonal Means Time Series (2017-2022)", 
                 gp = gpar(fontsize = 16, fontface = "bold"))
)
...
p_VPD_mk <- p_VPD_mk + labs(title = NULL)
p_rh_mk <- p_rh_mk + labs(title = NULL)
p_TA.TS <- p_TA.TS + labs(title = NULL)

# Remove legends from all (we'll add shared legend later)
p_SWC_noleg <- p_SWC_nf + theme(legend.position = "none")
p_VPD_noleg <- p_VPD_mk + theme(legend.position = "none")
p_rh_noleg <- p_rh_mk + theme(legend.position = "none")
p_TA_noleg <- p_TA.TS + theme(legend.position = "none")

# Extract legend from one plot
library(cowplot)
legend <- get_legend(p_TA.TS + theme(legend.position = "right", scale_color_manual(values = winteryear_colors, name = "Year")))

# Combine with fixed widths
combined <- (p_TA_noleg | p_SWC_noleg) /
            (p_VPD_noleg | p_rh_noleg) +
  plot_layout(
    widths = c(1, 1),  # Equal widths for left and right columns
    heights = c(1, 1)  # Equal heights for top and bottom rows
  ) +
  plot_annotation(
    title = "Seasonal Means Time Series (2017-2022)",
    theme = theme(
...
final_plot <- combined + legend +
  plot_layout(widths = c(10, 1))

print(final_plot)

# ggsave("seasonal_means_combined.png", final_plot, 
...
p_TA_noleg <- p_TA.TS + labs(title = NULL) + theme(legend.position = "none", axis.text.x = element_blank())
p_SWC_noleg <- p_SWC_nf + labs(title = NULL) + theme(legend.position = "none", axis.text.x = element_blank())
p_VPD_noleg <- p_VPD_mk + labs(title = NULL) + theme(legend.position = "none", axis.text.x = element_blank())
p_rh_noleg <- p_rh_mk + labs(title = NULL) + theme(legend.position = "none")

# Extract legend
legend <- get_legend(p_TA.TS + 
                       scale_color_manual(values = winteryear_colors, name = "Year") +
                       theme(legend.position = "right"))

# Combine plots in 2x2 grid  (use "=" for side by side, use "/" for stacking)
combined <- (p_TA_noleg) / (p_SWC_noleg) / 
            (p_VPD_noleg) / (p_rh_noleg) +
  plot_annotation(
    title = "Seasonal Means Time Series (2017-2022)",
...
# Add legend
final_plot <- wrap_plots(combined, as_ggplot(legend),
                         ncol = 2,
                         widths = c(10, 2))
...
```{r}
#LineaR REG 
# ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_MeanMet_vars_by_winteryear_plot_reduced.png", seasonal_MeanMet_by_winteryr.2, 
#         width = 15, height = 7, dpi = 600)

#MK
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_MeanMet_vars_by_winteryear_plot_reducedMK.png", final_plot, 
...
####Correcting the kendall's rank corr to mann-kendall test for timeseries data --streamlined by Claude
...
#annual budgets of all vars - timeseries 
```{r}
# Seasonal Trends Faceted by Season - All Variables
...
# Convert year to factor for consistent color mapping
annual_met$winteryear_factor <- as.factor(annual_met$winter_year)


annual_summary_met <- df_avg %>%
  filter(winter_year %in% 2017:2022) %>%
  group_by(winter_year) %>%
  summarise(
    TA_gapfilled_mean = mean(TA_gapfilled, na.rm = TRUE),
...
#MK stats - loop coding help from claude 

library(Kendall)
library(trend)

# List of variables to test
vars <- c("TA_gapfilled_mean", "TS_3_gapfilled_mean", "SWC_3_nf_mean",
          "VPD_mean", "RH_mean", "H_mean", "LE_mean", "G_mean")

# Run Mann-Kendall and Sen's slope for each variable
mk_results <- lapply(vars, function(v) {
  x <- na.omit(annual_summary_met[[v]])
  mk <- MannKendall(x)
  ss <- sens.slope(x)
  
  data.frame(
    variable   = v,
    tau        = round(as.numeric(mk$tau), 3),
    p_value    = round(as.numeric(mk$sl), 3),
    sens_slope = round(as.numeric(ss$estimates), 3)
  )
})
...
write_xlsx(mk_results_df, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/winteryear_annual_met_timeseries_statistics.xlsx")
```

#Figures

#annual RH timeseries 
```{r}
# RH 
annual_RH_stats<- ggplot(annual_met  %>%
                    filter(winter_year != "2016", winter_year != "2023"),
                    aes(x = winter_year, y = RH_mean, color = winteryear_factor)) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
  geom_point(size = 2.5) +
...
# annotate("text", x = 2021, y = 70,
#          label = expression(atop("MK tau = -1.00",
#                             atop("Sens slope = -6.32",
#                                  "p = 0.01"))),
#          size = 4.5)
...
```
#annual VPD timeseries 
```{r}
# VPD 
annual_VPD_stats<- ggplot(annual_met  %>%
                    filter(winter_year != "2016", winter_year != "2023"),
                    aes(x = winter_year, y = VPD_mean, color = winteryear_factor)) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
  geom_point(size = 2.5) +
...
annual_RH_VPD_timeseries <- annual_RH_stats + annual_VPD_stats

annual_RH_VPD_timeseries
```
...
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/annual_RH_VPD_timeseries_stats.png", annual_RH_VPD_timeseries, 
...
#this isn't the best diagnostic for this small dataset because n=6, so everything is coming up as normal/homogeneity of var when it may or may not be. Best to use non-parametric here but leaving this code because it's a good reference for automation of the residual plotting 
