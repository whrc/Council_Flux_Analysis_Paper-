...
# Trends in FC and FCH4 - using winter adj budgets for FC and FCH4
...
p_FC <- ggplot(adj_C_budgets) +
  ...
  geom_point(aes(x = Year, y = Net_CO2_adj_dailyflux, color = year_factor, group = Year), 
    ...
    geom_errorbar(aes(x = Year,
                    ymin = Net_CO2_adj_dailyflux - Net_CO2_adj_dailyflux_se, 
                    ymax = Net_CO2_adj_dailyflux + Net_CO2_adj_dailyflux_se),
...
p_FCH4 <- ggplot(adj_C_budgets) +
  ...
  geom_point(aes(x = Year, y = Net_CH4_adj_dailyflux, color = year_factor, group = Year), 
             size = 3) +
   geom_errorbar(aes(x = Year,
                    ymin = Net_CH4_adj_dailyflux - Net_CH4_adj_dailyflux_se, 
                    ymax = Net_CH4_adj_dailyflux + Net_CH4_adj_dailyflux_se),
                width = 0.2, linewidth = 0.8) +
...
#save images
```{r}
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_mean_adjFC_by_year.png", p_FC,
       width = 8, height = 5, dpi = 600)

ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_mean_adjFCH4_by_year.png", p_FCH4,
       width = 8, height = 5, dpi = 600)
```



#Autocorr - seasonal FC and FCH4
...
FC_fall <- adj_C_budgets%>%
  filter(Season == "Fall Senescence")
#autocorrelation - want everything below the blue dotted line 
acf(FC_fall$Net_CO2_adj_dailyflux, na.action = na.omit, main = "Autocorrelation of variable")
#partial autocorr - removes effect of intermediate lags, still want everything below blue dotted line 
pacf(FC_fall$Net_CO2_adj_dailyflux, na.action = na.omit, main = "Partial Autocorrelation")
...
Box.test(FC_fall$Net_CO2_adj_dailyflux, lag = 20, type = "Ljung-Box") #p<0.001
...
lm_test<- lm(Net_CO2_adj_dailyflux ~ Year, data = FC_fall)
dwtest(lm_test)
...
FC_results <- test_seasonal_trends(adj_C_budgets, 
                                    "Net_CO2_adj_dailyflux", 
                                    "Daily Avg half-hourly CO2 Flux")
...
FCH4_results <- test_seasonal_trends(adj_C_budgets, 
                                     "Net_CH4_adj_dailyflux", 
                                     "Daily Avg half-hourly CH4 Flux")
...
all_results <- rbind(FC_results, FCH4_results)
...
acf(na.omit(growing_data$RH_mean, lag.max = 2, plot = TRUE))
...
lm_test<- lm(RH_mean ~ year, data = growing_data)
dwtest(lm_test)
acf(residuals(lm_test), main = "ACF of model residuals")
