...
#Coding help w Claude
```{r}
# Step 2a: Annual Trends Analysis - Complete Visualization Script
...
cat("Shapiro-Wilk p-value: > 0.05 suggests normality\n") #but important to note that with n=6, shapiro wilkes has VERY low power, so rely on QQplot & histogram & use your judgement 
...
##### starts with testing lm assumptions, pivots to non-parametric if assumptions are violated, used kendall's tau (similar to spearman corr but supposedly better for small sample sizes and here we only have 6 yrs so n = 6) - streamlined function code by Claude 
```{r}
...
print(gf_ta_results[, c("season", "mean_value", "normal", "slope", "slope_p", 
                     ...
                     "percent_change_2017_2022", "significant")])
...
significant_trends <- all_results %>%
  ...
  select(variable, season, mean_value, units, change_per_year, 
         percent_change_2017_2022, slope_p, kendall_p, significant)
...
p_VPD_with_stats <- ggplot(seasonal_summary_all, aes(x = year, y = VPD_mean, color = year_factor)) +
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
  geom_point(size = 2.5) +
  ...
  # Add only slope, R², and p-value (no full equation)
  stat_poly_eq(aes(label = paste("Slope ==", 
                                   after_stat(sprintf("%.3f", coef.ls[[1]][[2]])),
                                   "~~~R^2 ==", 
                                   after_stat(sprintf("%.2f", r.squared)),
                                   "~~~italic(p) ==",
                                   after_stat(sprintf("%.3f", p.value)),
                                   sep = "~")),
               formula = y ~ x, 
               parse = TRUE,
               size = 3,
...
#pairwise comparisons of years using daily avg data 
```{r}
...
#### note that here it's using an intercept based off of year = 0, when our first year is 2017...so the b/y-intercept here is wrong 
...
# Example for VPD which showed significant trends in all seasons
...
#Plotting vars with sig linear trends over the years
```{r}
 ...
 # Annotations
  annotate("text", x = 2018, y = 8, label = "R² = 0.58", size = 3) +
  annotate("text", x = 2018, y = 7.6,label = "Slope = 0.81", size = 3) 
