...
#first day of Winter: DOY - NA, data incomplete 
#first day of Fall: DOY 234, 8/22 maybe, but jumps back to neg NEE, data incomplete can't verify
...
df_avg_2023 <- df_avg_2023 %>%
  mutate(
       season = case_when(
      #(DOY >= 246 & DOY <= 284) ~ 'Fall Senescence',  -- NA
     # (DOY >= 157 & DOY <= 245) ~ 'Growing Season', have start of growing but not end 
     # (DOY >= 289 | DOY <= 144) ~ 'Winter',
         (DOY >= 1 & DOY <= 156) ~ "Winter",
         (DOY >= 157) ~ "Growing Season",
...
####2023 Seasonal Plot with NEE and temp - NA, not enough data to complete, only here for reference 
```{r}
#2023
...
#MONSTER Seasonal plot 2017 - 2023 
```{r}
#Plot of all years 
...
df<- df %>%
  mutate(
    season = case_when(
      ...
      # 2023 incomplete but adding winter for 2022-2023 winter budget
      year == "2023"  & (DOY >= 157) ~ 'Growing Season',
      year == "2023" & (DOY >= 1 & DOY <= 156) ~ "Winter",
...
#Create Winter_year to group by continuous winters that span across years (ex: 2019-2020)

#### df_avg with daily avg 
```{r}
...
#Save dataset with winter_year 
```{r}
# # Save as csv
write.csv(x = df_avg3, file = 'C:/Users/kkent/Documents/Council Data/Council BASE gapfilling/council_AVG_gapfilled_clean_2017_2023_for analysis.5.csv',quote = F,row.names = F)
...
#Save HH dataset with winter_year 
```{r}
# # Save as csv
...
#Season Length Figure 

#Number of days within season 
```{r}
library(tidyverse)

# Count days per season per winter_year
season_counts <- df_avg %>%
  filter(!is.na(season) & !is.na(winter_year)) %>%
  filter(winter_year != "2023", winter_year != "2016") %>%
  group_by(winter_year, season) %>%
  summarise(n_days = n(), .groups = "drop")

# Plot
ggplot(season_counts, aes(x = factor(winter_year), y = n_days, fill = season)) +
  geom_col() +
  facet_wrap(~season) +
  labs(
    x = "Winter Year",
    y = "Number of Days",
    title = "Days per Season by Winter Year"
  ) +
  theme_bw() +
  theme(
...
#Adjusting the winter label 
```{r}

# Order seasons (if not already done)
season_counts <- season_counts%>%
  mutate(season = factor(season, levels = c("Growing Season", "Fall Senescence", "Winter")))



# Create the continuous winter year label
season_counts <- season_counts %>%
  mutate(x_label = ifelse(season == "Winter", 
                         paste0(winter_year, " - ", winter_year + 1),
                         as.character(winter_year)))

# Plot
season_length_fig <- ggplot(season_counts, aes(x = x_label, y = n_days, fill = season)) +
  geom_col() +
  facet_wrap(~season, scales = "free_x") +  # Add scales = "free_x" to allow scales to differ
  #manually set color 
  scale_fill_manual(values = c(
     "Growing Season" = "#4caf50",    # Green
    "Fall Senescence" = "#ff7f50",   # Salmon
    "Winter" = "#5b9bd5"   #Blue
    )) +              
  ...
  labs(
    x = "Year",
    y = "Number of Days",
    title = "Days per Season by Year",
    subtitle = "Features continuous winters spanning from one year into following year"
  ) +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, face = "bold"),
...
#Save seasonal lengths figure 

```{r}
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/winteryear_season_length_fig.png", season_length_fig, 
        width = 8, height = 5, dpi = 600)
```
