---
title: "Council_WD_DailyAvg_Calc" #Adding in prevailing winds of the day based on WD frequency count so it can be added to the df_avg (daily averaged dataset)
#Included the first method for reference, scroll down to new method which uses the 22.5 degree offset to better cover the binned cardinal directions 
#In df_avg dataset, this will have a columns for the original way and WD_modal2 & WD_cardinal2 are from the new, adjusted way 
...
####Important to note that the below method will skew results. For example, it would set N as 0 - 45, but 45 is NE, and it would set NE as 45 - 90, but 90 is E, and so on. See below "Adjusted Method" for using the 22.5 degree offset to better capture binned cardinal directions* 
```{r}
...
```
# *** ADJUSTED METHOD ***
#New way, help from Claude, slightly offset cardinal binning by 22.5 degrees to better capture the cardinal directions
...
# Filter to one day (change date as needed)
df_day <- df %>%
  filter(as.Date(date) == as.Date("2018-07-21"))
...
# See which actual degree values got classified as a cardinal direction - shows you the actual measured degrees put into this cardinal direction bin 
se_values <- df_test$WD[!is.na(df_test$WD)]
nw_degrees <- nw_values[nw_values >= 292.5 & nw_values < 337.5]
se_degrees <- se_values[se_values >= 112.5 & se_values < 157.5]
print("Degrees classified as NW:")
print(sort(se_degrees))
...
# # Check the results - coding help from Claude 
...
# 
#  Optional - Create a summary to compare old vs new wind data
