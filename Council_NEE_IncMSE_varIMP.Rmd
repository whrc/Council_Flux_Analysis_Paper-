...
df_cc = df[complete.cases(df$FC, df$TA, df$WD, df$WD_Cardinal, df$RH, df$NETRAD, df$WS, df$TS_3_1_1, df$SWC_3_1_1, df$H, df$LE, df$VPD, df$G_1_1_1),]
...
boruta = Boruta(FC ~ TA + RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1 + TS_SWC_interact, data = df_cc, doTrace = 2,maxRuns = 100)
boruta$finalDecision #indicates all above are confirmed important variables via a list 
plot(boruta,las = 2) 
...
site.nee_full.cc <- randomForest(FC ~ TA + RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1 + TS_SWC_interact,
                             data = df_cc,
                             ntree = 850,
                             importance = TRUE)


 vif_model <- lm(FC ~ TA + RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1 + TS_SWC_interact, data = df_cc)
# 
 vif(vif_model) 
      #        TA              RH          NETRAD              WS              WD 
      # 12.886858        3.632409        7.657315        1.234438        1.133425 
      #  TS_3_1_1       SWC_3_1_1             VPD              LE               H 
      # 31.915917        1.915787        9.239496        4.614354        6.577908 
...
site.nee_1.cc <- randomForest(FC ~ TA + RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1,
                             data = df_cc,
                             ntree = 850,
                             importance = TRUE)


 vif_model <- lm(FC ~ TA + RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1, data = df_cc)
# 
 vif(vif_model) 
#        TA        RH    NETRAD        WS        WD  TS_3_1_1 SWC_3_1_1       VPD        LE 
# 12.794332  3.572429  7.656663  1.232958  1.127200  7.999954  1.669048  9.239237  4.473738 
#         H   G_1_1_1 
#  6.461418  4.940612 
...
site.nee_2.cc <- randomForest(FC ~ RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1,
                             data = df_cc,
                             ntree = 850,
                             importance = TRUE)


 vif_model <- lm(FC ~  RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + VPD + LE + H + G_1_1_1, data = df_cc)
# 
 vif(vif_model) 
 #       RH    NETRAD        WS        WD  TS_3_1_1 SWC_3_1_1       VPD        LE         H 
 # 3.114736  7.250680  1.232892  1.112406  5.785780  1.570598  7.372138  4.462798  6.136320 
 #  G_1_1_1 
 # 4.168524 
...
site.nee_3.cc <- randomForest(FC ~ RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + LE + H + G_1_1_1,
                             data = df_cc,
                             ntree = 850,
                             importance = TRUE)


 vif_model <- lm(FC ~  RH + NETRAD + WS + WD + TS_3_1_1 + SWC_3_1_1 + LE + H + G_1_1_1, data = df_cc)
# 
 vif(vif_model) 
 #       RH    NETRAD        WS        WD  TS_3_1_1 SWC_3_1_1        LE         H   G_1_1_1 
 # 1.175159  7.155105  1.227465  1.111692  4.129575  1.531578  4.427206  5.933628  3.430182 
...
```
##### reduced site model 6 - removing TS-SWC interact, TA, VPD, NETRAD, G, LE
```{r}
#TS_3 is highest, but going to remove G first since it's likely collinear with that and TS is more important than G 

site.nee_6.cc <- randomForest(FC ~ RH + WS + WD + TS_3_1_1 + SWC_3_1_1 + H,
                             data = df_cc,
                             ntree = 850,
                             importance = TRUE)
