...
df <- df %>%
  mutate(monthday  = as.Date(format(TIMESTAMP,   # force every point into a "2000‐MM-DD" date
                      "2000-%m-%d")))
...
### Exp. Model for soil temp v C fluxes - asll seasons 
```{r}
...
```{r}
#β-1 (the B1TS) captures the linear change in methane flux per °C
#β-2 (the B2(TS^2) captures how that rate accelerates or decelerates as temperature rises (the curvature)
...
ggplot() +
  ...
  labs(title = "Quadratic Model: Methane Flux vs. Soil Temperature",
       subtitle = paste("FCH4 =", 
                      ...
                      round(coef(quad_model_ch4)[2], 2), "× Temp +", 
                      round(coef(quad_model_ch4)[3], 2), "× Temp²"),
  ...
  for (i in seq_along(temp_cols)) {
    ...
    p <- ggplot(df, aes_string(x = temp_col, y = flux_var)) +
      ...
      labs(
        ...
        y = if(flux_var == "FC_night") "Nighttime CO2 Flux (μmol/m²/s)" else "CH4 Flux (nmol/m²/s)"
      )
...
ggplot(df_avg_nowinter, aes(x = TS_3_1_1, y = FC_night, color = season)) +
  ...
  theme_bw() +
  labs(x = "Soil Temperature (°C)", y = "Nighttime CO2 Flux (μmol/m²/s)", 
...
ggplot(df_avg_nowinter, aes(x = TS_3_1_1, y = FCH4 * (1/1000), color = season)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw() +
  labs(x = "Soil Temperature (°C)", y = "CH4 Flux (μmol/m²/s)", 
...
ggplot(df_avg, aes(x = TS_3_1_1, y = FC_night, color = SWC_3_1_1)) +
  ...
  geom_smooth(method = "auto", se = TRUE) +
  theme_bw() +
  labs(x = "Soil Temperature (°C)", y = "FC_night Flux (μmol/m²/s)", 
...
summary(quad_swc_model_ch4)
summary(interactive_model_ch4)
```
...
ggplot() +
  ...
  annotate("text", x = min(df_avg$TS_3_1_1, na.rm = TRUE) + 2, 
          ...
          label = paste("Model: FCH4 =", 
                       ...
                       round(coef(quad_model_ch4)[2], 2), "× Temp +", 
                       round(coef(quad_model_ch4)[3], 3), "× Temp²\n",
...
temp_bins <- c(-10, -3, 0, 4, 11, 18)
temp_labels <- c("Below Freezing (-10 to -3°C)",
                "Around Freezing (-3 to 0°C)",
                "Just Above Freezing (0 to 4°C)",
                "Cool (4 to 11°C)",
                "Warm (11 to 18°C)")
...
ggplot(df_avg, aes(x = TA , y = FCH4, color = SWC_3_1_1)) +
  ...
  geom_smooth(method = "auto", se = TRUE) +
  theme_bw() +
  labs(x = "Air Temp(°C)", y = "CH4 Flux (μmol/m²/s)", 
...
ggplot(df_avg, aes(x = TA , y = FC_night, color = SWC_3_1_1)) +
  ...
  geom_smooth(method = "auto", se = TRUE) +
  theme_bw() +
  labs(x = "Air Temp(°C)", y = "FC_night Flux (μmol/m²/s)", 
...
ggplot(df_avg_nowinter, aes(x = SWC_3_1_1, y = FC_night, color = season)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw() +
  labs(x = "SWC", y = "Nighttime CO2 Flux (μmol/m²/s)", 
...
ggplot(df_avg_nowinter, aes(x = SWC_3_1_1, y = FCH4, color = season)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  theme_bw() +
  labs(x = "SWC", y = "CH4 Flux (μmol/m²/s)", 
    ...
    if (nrow(df_year) > 0) {
      p <- ggplot(df_year, aes_string(x = temp_var, y = flux_var)) +
        ...
        labs(x = "Soil Temperature (°C)", 
             y = if(flux_var == "FC_night") "CO2 Flux (μmol/m²/s)" else "CH4 Flux (nmol/m²/s)")
...
ggplot(df_avg %>% filter(complete.cases(WD_cardinal, FC_night)), aes(x = WD_cardinal, y = FC_night)) +
  ...
  geom_hline(yintercept=0)+
  labs(x = "Wind Direction", y = "Nighttime CO2 Flux (μmol/m²/s)", 
