...
#dforig$date <- as.POSIXct(df$date, format = "%Y-%m-%d %H:%M:%OS") #yr month day hour min sec
...
#comparing to previous data using the SWC1
# df_2017orig <- year_df2(dforig, 2017)
# df_2019orig <- year_df2(dforig, 2019)

#map out the timestamp of the monthly dataset  --> not currently using, have been using aggregate functions in codes below but keeping for reference 
...
#df_avg_2023$date <- as.Date(df_avg_2023$date)

# Extract the year and month to create a grouping column
df_avg_2023$year_month <- format(df_avg_2023$date, "%Y-%m")

# Calculate the sum of carbon emissions (g C/m²/month) for each month - this is not working now...
monthly_CO2sum_2023 <- aggregate(FC_F_no_NAs ~ year_month, data = df_avg_2023, sum, na.rm = TRUE)
monthly_CH4sum_2023 <- aggregate(FCH4_F_no_NAs ~ year_month, data = df_avg_2023, sum, na.rm = TRUE)
...
monthly_Cflux_sum_2023 <- aggregate(cbind(FC_F_no_NAs, FCH4_F_no_NAs) ~ year_month, data = df_avg_2023, sum, na.rm = TRUE)

# View the resulting dataframe with monthly sums
monthly_Cflux_sum_2023

#save table of monthly C flux budgets per year 
#write.csv(monthly_Cflux_sum_2023, file = "monthly_Cflux_sum_2023.csv", row.names = FALSE)

#double check the monthly sums 
#extract month
df_avg_2023$month <- as.numeric(format(df_avg_2023$date, "%m"))
 
#filter out by month - October = 10
sum_october_2023 <- df_avg_2023[month == 10, .(monthly_sum = sum(FC_F_no_NAs, na.rm = TRUE))]

# Print the result
sum_october_2023

#sum - check sum to match yearly budget 
sum(df_avg_2023$FC_F_no_NAs)
sum(df_avg_2023$FCH4_F_no_NAs)
...
monthly_Cflux_sum_2023$month_name <- format(as.Date(paste0(monthly_Cflux_sum_2023$year_month, "-01")), "%B")

#sum of CO2 and CH4 fluxes
monthly_Cflux_sum_2023 <- monthly_Cflux_sum_2023 %>%
...
# monthly_Cflux_sum_2023_long <- monthly_Cflux_sum_2023 %>%
...
# ggplot(monthly_Cflux_sum_2023_long, aes(x = factor(month_name, levels = month.name), y = Flux_Sum, fill = Flux_Type)) +
...
#        title = "Monthly CO2 and CH4 Flux Sums for 2023",
#        fill = "C flux") +
#   theme_minimal() +
...
scaling_factor <- 30
monthly_Cflux_sum_2023$FCH4_scaled <- monthly_Cflux_sum_2023$FCH4_F_no_NAs * scaling_factor
...
monthlyC_2023_barplot <- ggplot(monthly_Cflux_sum_2023_long, aes(x = factor(month_name, levels = month.name), y = Flux_Sum, fill = Flux_Type)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(
  ...
  scale_y_continuous(
    ...
    #y = expression(CH[4]~Flux~(nmolCH[4]~m^-2~y^-1)),
  labs(x = "Month", y = "C Flux (g/m²/month)", 
       title = "Monthly C Fluxes for 2023",
       #title = "Monthly CO2 and CH4 Flux Sums for 2023",
       fill = "C flux") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

monthlyC_2023_barplot 
...
## Plot with legend- use only for legend
seasonalC_2019_plot <- ggplot(data = df_avg_2019) +
  theme_bw() +
...
# annotate("text", size = 3, x = as.POSIXct("2023-10-10"), y = -4, label = "Senescence", color = "black") +
...
#TO DO
```{r}
#don't think data is complete enough to calc this year's budget -- not yet done for 2023
# Add seasonal delineations to the daily average dataframe 
...
####tried to incorporate those in yearly code chunks above so this is not needed

``` {r}
# 
# #will have to do this for every year since I have multi-yr data, and each year will vary a little
# 
...
```{r}
seasonal_integrals <- function(df_2020) {
...
seasonal_integrals_2020 <- seasonal_integrals(df_2020)

#reorders seasons to timeline order
seasonal_integrals_2020$season <- factor(seasonal_integrals_2020$season, levels = c("Growing Season", "Fall Senescence", "Winter"))

seasonal_2020 <-ggplot(seasonal_integrals_2020, aes(x = season, y = integral_value)) +
  theme_bw()+
  geom_hline(yintercept=0, colour="black")+
  geom_bar(stat = "identity", position = "dodge", fill = "turquoise3") +
  labs(
    ...
    title = expression("Net CO"[2] * " Flux by Season: 2020")
  ) +
  geom_label(aes(label = integral_value), vjust = ifelse(seasonal_integrals_2020$integral_value >= 0, -0.7, 1.5), colour = "black", fill ="white")+
