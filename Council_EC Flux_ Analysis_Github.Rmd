...
ggplot(data = df_avg_2017, aes(x=date))+
  theme_bw()+
...
ggplot(net_wp_2019, aes(x = Category, y = Value)) +
  theme_bw()+
  ...
  labs(
    ...
    y = expression(Net~Carbon~Flux~(gCO[2]-C~m^-2~y^-1))) +
  geom_label(aes(label = Value), vjust = ifelse(net_wp_2019$Value >= 0, -0.7, 1.5), colour = "black", fill ="white")+
  scale_y_continuous(
    #limits=c(-50, 20),
     breaks = seq(-20, 20, 5),  # changing for 2019 
    limits = c(-20, 20))+ #changing for year 2019
      #breaks = seq(-75, 30, 5),  # Set limits for the primary axis - other years go up to -73
    #limits = c(-75, 20))+ 
  labs(title = "Cumulative Annual Carbon Budget for 2019 (January 1 - December 31)") #for 2019
#labs(title = "2019")
```
...
ggplot(monthly_Cflux_sum_2019_long, aes(x = factor(month_name, levels = month.name), y = Flux_Sum, fill = Flux_Type)) +
  ...
  scale_y_continuous(
    ...
    name = expression(CO[2]~Flux~(gCO[2]-C~m^-2~month^-1)),
       breaks = seq(-55, 30, 5),  # Set limits for the primary axis
    limits = c(-55, 30),  # Explicitly set limits for the primary axis
    sec.axis = sec_axis(~ . / scaling_factor, name = expression(Methane~Flux~(gCH[4]-C~m^-2~month^-1)),breaks = seq(-1, 1, 0.25))
  ) +
    #y = expression(CH[4]~Flux~(nmolCH[4]~m^-2~y^-1)),
  labs(x = "Month", y = "C Flux (g/m²/month)", 
...
ggplot(data = df_avg_2019) +
  ...
  # lines for soil and air temperature 
  geom_line(aes(x = day, y = TS_3_1_1/15, colour = "15cm Soil Temperature")) + #divided to scale
...
```{r}
ggsave(
  filename = "2017-2023_Seasonal_Trends.png",  # File name and extension
