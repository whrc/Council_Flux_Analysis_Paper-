...
#Comparing SWC and soil temp from the different locations / probes 
```{r}
#SWC 
ggplot(data = df) +
...
write.csv(seasonal_budgets, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_budgets.csv", row.names = FALSE)
...
#using WD modal because corr matrix can only use numeric variables, so can't use WD_cardinal ; use WD_modal2 to reflect the updated/corrected (?) wind directions 
env_flux_daily_notGF <- df_avg %>%
  select(FC, FCH4, GPP, RECO, TS_3_1_1, SWC_3_1_1, TA, WS, WD_modal, WD_modal2, TA_ERA5, TS_ERA5) %>%
  rename(
    NEE = FC,
    CH4_flux = FCH4,
    GPP = GPP,
    ...
    Air_temp = TA,
    Wind_speed = WS,
    #Wind_Direction = WD, #this is just WD degrees avg'd, which is not a true rep of avg daily wind dir 
    WindDir_Modal = WD_modal,
    WindDir_Modal2 = WD_modal2,
...
# different way to add significance indicators - removes NAs by 1:1 corr 
cor_long$sigBon <- ""
cor_long$sigBon[cor_long$p_adjBon < 0.05] <- "*"    # p < 0.05
cor_long$sigBon[cor_long$p_adjBon< 0.01] <- "**"   # p < 0.01
cor_long$sigBon[cor_long$p_adjBon < 0.001] <- "***" # p < 0.001
...
#Adjusting tiles text color based on corr strength 

#Benjamini-Hochberg  correction 
ggplot(cor_long, aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  # Add correlation values for EITHER strong correlations OR significant ones
  geom_text(data = subset(cor_long, abs(Correlation) > 0.3 | p_adjBH < 0.05), #the | represents "OR" so this displays correlations above 0.3 OR significant correlations; also use "abs" to use absolute value of the corr so it's printed whether it's positive or negative*  
            aes(label = paste0(sprintf("%.2f", Correlation), sigBH),
                color = abs(Correlation) < 0.5), # Changes text color based on correlation strength
            size = 4.5,
            fontface = "bold") + #to make numbers in tiles bold
  scale_fill_gradient2(low = "red", 
...
ggplot(cor_long, aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
  # Add correlation values for EITHER strong correlations OR significant ones
  geom_text(data = subset(cor_long, abs(Correlation) > 0.3 | p_adjBon < 0.05), #the | represents "OR" so this displays correlations above 0.3 OR significant correlations; also use "abs" to use absolute value of the corr so it's printed whether it's positive or negative*  
 ...
 #saves the figure directly above 
# ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/flux_Spearmancorrelation_matrix_allyearsBONadj_notGF.png",
#        width = 11, height = 8, dpi = 600)

```


#Corr matrix cont'd - removing the duplicates - claude help
```{r}
# Load libraries
library(Hmisc)
library(reshape2)
...
# Function to remove duplicate correlations (keep only upper triangle)
remove_duplicates <- function(cor_data) {
  # Create a logical matrix for upper triangle (excluding diagonal)
  cor_matrix <- acast(cor_data, Var1 ~ Var2, value.var = "Correlation")
  upper_tri <- upper.tri(cor_matrix, diag = FALSE)
  
  # Convert back to indices
  indices <- which(upper_tri, arr.ind = TRUE)
  row_names <- rownames(cor_matrix)[indices[,1]]
  col_names <- colnames(cor_matrix)[indices[,2]]
  
  # Filter the original data to keep only upper triangle
  cor_data_filtered <- cor_data %>%
    filter(paste(Var1, Var2) %in% paste(row_names, col_names))
  
  return(cor_data_filtered)
}

# Alternative function to keep lower triangle instead
remove_duplicates_lower <- function(cor_data) {
  cor_matrix <- acast(cor_data, Var1 ~ Var2, value.var = "Correlation")
  lower_tri <- lower.tri(cor_matrix, diag = TRUE)  # Include diagonal
...
# Function to create a rectangular (non-square) correlation matrix
create_rectangular_matrix <- function(cor_data, flux_vars, env_vars) {
  # Only keep correlations between flux variables and environmental variables
  # Plus correlations among flux variables
  cor_data_rect <- cor_data %>%
    filter((Var1 %in% flux_vars & Var2 %in% env_vars) |
           (Var1 %in% env_vars & Var2 %in% flux_vars) |
           (Var1 %in% flux_vars & Var2 %in% flux_vars & Var1 != Var2)) %>%
    # Remove duplicates by keeping only one direction for each pair
    filter(!(Var1 %in% env_vars & Var2 %in% flux_vars)) %>%  # Remove env->flux, keep flux->env
    distinct()
  
  return(cor_data_rect)
}

# Create base theme for consistency
correlation_theme <- theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 12, face = "bold"),  # Environmental variables at bottom
    axis.text.y = element_text(size = 12, face = "bold"),  # Carbon flux variables on right
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    legend.title = element_text(size = 12, face = "bold")
  )

# Combined carbon flux correlations
flux_vars <- c("NEE", "CH4_flux", "GPP", "RECO")
env_vars <- c("Soil_temp", "Air_temp", "Soil_moisture", "Wind_speed", "WindDir_Modal", "WindDir_Modal2", 
              "Air_temp_ERA5", "Soil_temp_ERA5")  # Adjust these to match your actual variable names

# Option 1: Try lower triangle instead of upper triangle
cor_all_flux_lower <- cor_long %>%
  filter((Var1 %in% flux_vars & Var2 %in% env_vars) |
         (Var1 %in% env_vars & Var2 %in% flux_vars) |
         (Var1 %in% flux_vars & Var2 %in% flux_vars)) %>%
  remove_duplicates_lower()

# Option 2: Create a cleaner rectangular matrix
cor_all_flux_rect <- create_rectangular_matrix(cor_long, flux_vars, env_vars)

# Plot Option 1: Lower triangle (empty spaces on right)
plot_lower <- ggplot(cor_all_flux_lower, aes(x = Var2, y = Var1, fill = Correlation)) +
  geom_tile() +
  geom_text(
    data = subset(cor_all_flux_lower, abs(Correlation) > 0.3 | p_adjBon < 0.05),
    aes(label = paste0(sprintf("%.2f", Correlation), sigBon),
  ...
  correlation_theme +
  labs(
    title = "Carbon Flux Correlations (Lower Triangle)",
    fill = "Spearman's \nCorrelation \n(ρ)",
    x = "", y = ""
  )

# Plot Option 2: Rectangular matrix (cleaner layout)
plot_rectangular <- ggplot(cor_all_flux_rect, aes(x = Var2, y = Var1, fill = Correlation)) +
  geom_tile() +
  geom_text(
    data = subset(cor_all_flux_rect, abs(Correlation) > 0.3 | p_adjBon < 0.05),
    aes(label = paste0(sprintf("%.2f", Correlation), sigBon),
        color = abs(Correlation) < 0.5),
    size = 4.5, fontface = "bold"
  ) +
  ...
  correlation_theme +
  labs(
    title = "Carbon Flux Correlations with Environmental Variables",
    fill = "Spearman's \nCorrelation \n(ρ)",
    x = "", y = ""
  )

# Display both options
print("Option 1: Lower triangle")
print(plot_lower)
print("Option 2: Rectangular matrix")
print(plot_rectangular)
```

```{r}
 #saves the figure directly above 
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/flux_Spearmancorrelation_matrix_allyearsBONadj_notGF_rectangular_2.png",plot_rectangular ,
...
adj_annual_Cbudget_bargraph <- ggplot(adj_C_budgets_long %>%
  ...
  labs(title = "Annual Net Carbon Flux by Year",
       x = "Year",
       y = expression(Net~C~Flux~(gC/m^2)),
       fill = "Flux Type") +
  scale_fill_manual(values = c("salmon", "turquoise", "grey"),
                    labels = c(expression(CH[4]), expression(CO[2]), "Total C")) +
   scale_y_continuous(
        breaks = seq(-15, 45, 10),
        limits = c(-5, 45)) +  # Slightly increased upper limit to accommodate labels
  theme_bw()+
    theme(
      plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),
      axis.text = element_text(size = 12, face = "bold"),
...
adj_grid_arranged <- arrangeGrob(
  ...
  ncol = 2,  # Arrange plots in 2 columns
  # Add shared y-axis label, rotated 90 degrees
  left = textGrob(expression(Net~Carbon~Flux~(gC~m^-2)), 
                  rot = 90,
                  gp = gpar(fontsize = 30))
)
...
#Adjusting winter CH4 based on daily mean fluxes

#### calc daily mean flux of CH4 for growing season for each year, find proportion of winter faily mean flux based on 2019 --> multiply by # of days within the winter season for that year 

#Daily mean FCH_F per season 
```{r}
# CH4 in nano-moles of CH4/m^2/s)
#Net CH4 Flux - gapfilled

# Calculate daily average CH4 flux (FCH4_F) by season with standard error
seasonal_FCH4_summary_gf <- df_avg %>%
  group_by(year, season) %>%
  summarize(
    mean_flux = mean(FCH4_F, na.rm = TRUE),
    se_flux = sd(FCH4_F, na.rm = TRUE)/sqrt(sum(!is.na(FCH4_F))),
    n_days = n()  # number of days per season
  )

# Format the results with mean ± SE
seasonal_FCH4_formatted_gf <- seasonal_FCH4_summary_gf %>%
  mutate(
    flux = sprintf("%.2f ± %.2f", mean_flux, se_flux)
  ) %>%
  select(season, flux, n_days)
         
seasonal_FCH4_formatted_gf

# Save to Excel
library(writexl)
write_xlsx(seasonal_FCH4_formatted_gf, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_FCH4_summary.GF.xlsx")

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#NOT GAP FILLED

seasonal_FCH4_summary <- df_avg %>%
  group_by(year, season) %>%
  summarize(
    mean_flux = mean(FCH4, na.rm = TRUE),
    se_flux = sd(FC, na.rm = TRUE)/sqrt(sum(!is.na(FCH4))),
    n_days = n()  # number of days per season
  )


# Format the results with mean ± SE
seasonal_FCH4_formatted <- seasonal_FCH4_summary %>%
  mutate(
    flux = sprintf("%.2f ± %.2f", mean_flux, se_flux)
  ) %>%
  select(season, flux, n_days)
         
seasonal_FCH4_formatted

# Save to Excel
library(writexl)
write_xlsx(seasonal_FCH4_formatted, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_FCH4_summary.xlsx")
...
#based on 2019 dailg avg growing season fluxes, winter daily avg CO2 flux can be ~44% of growing season daily avg flux (0.445) --> multiply winter daily avg fluxes by 0.445 & re-calc the daily avg CH4 flux to get an adjusted potential winter daily avg flux 
...
```{r}
library(readxl)
adj_C_budgets <- read_xlsx(path ="C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/Seasonal_Cbudgets_Table_all_years_for_paper.xlsx", sheet = "R_template_adjWinterC" )


# Reshape data for plotting
adj_C_budgets_long<- adj_C_budgets %>%
  pivot_longer(cols = c(Net_CH4_adj_dailyflux, Net_CO2_adj_dailyflux, Total_Net_C_adj_dailyflux
),
               names_to = "flux_type",
               values_to = "flux")
...
#Plot of budgets with winter-adj CH4 
```{r}
...
adj_seasonal_budgets_CH4 <- ggplot(adj_C_budgets %>% 
                                    filter(Season != "Annual"), #takes out "annual" as a season
                                  aes(x = factor(Year), y = Net_CH4_adj_dailyflux, fill = Season)) +
  geom_bar(stat = "identity", position = "dodge") +
  #labs(title = "Seasonal CH4 Budgets by Year",
  labs(title = expression(bold(Net~Seasonal~CH[4]~Flux~by~Year)),
       x = "Year",
...
# #Save CH4 seasonal budgets 
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/adj_seasonal_budgets_CH4_dailyflux.png", adj_seasonal_budgets_CH4,
       width = 10, height = 7, dpi = 600)
```
...
adj_seasonal_budgets_CO2 <- ggplot(adj_C_budgets %>% 
                                    filter(Season != "Annual"), #takes out "annual" as a season
 aes(x = factor(Year), y = Net_CO2_adj_dailyflux, fill = Season)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_hline(yintercept=0, colour = "black")+ 
  labs(title = expression(bold(Net~Seasonal~CO[2]~Flux~by~Year)),
       x = "Year",
  ...
  ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/adj_seasonal_budgets_CO2_dailylfux.png", adj_seasonal_budgets_CO2, 
       width = 10, height = 7, dpi = 600)
```
...
#bar graph of seasonal budgets with winteradj daily 
```{r}
#bar graph of net CH4, CO2, and total C without labels above bars 
...
#save
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/adjwinter_yearly_carbon_fluxes.png", adj_annual_Cbudget_bargraph,
       width = 10, height = 7, dpi = 600)
```
...
#GAP FiLLED
seasonal_NEE_summary_gf <- df_avg %>%
  group_by(year, season) %>%
  summarize(
...
# Format the results with mean ± SE
seasonal_NEE_formatted_gf <- seasonal_NEE_summary_gf %>%
  mutate(
    flux = sprintf("%.2f ± %.2f", mean_flux, se_flux)
  ) %>%
  select(season, flux, n_days)
         
seasonal_NEE_formatted_gf

# Save to Excel
library(writexl)
write_xlsx(seasonal_NEE_formatted_gf, "C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/seasonal_NEE.GF_summary.xlsx")
...
seasonal_NEE_summary <- df_avg %>%
  group_by(year, season) %>%
  summarize(
    mean_flux = mean(FC, na.rm = TRUE),
    se_flux = sd(FC, na.rm = TRUE)/sqrt(sum(!is.na(FC))),
    n_days = n()  # number of days per season
  )
...
#Testing ERA5 soil vs Tsoil overall, & above and below 0 deg


```{r}
summary(lm(df$TS_3_1_1 ~ df$TS_ERA5)) #R2 = 0.861; slope = 1.01
summary(lm(df_avg$TS_3_1_1 ~ df_avg$TS_ERA5)) #R2 = 0.9368; slope = 1.01


#df
Tsoil_temp_test= ggplot(data = df,aes(TS_3_1_1,TS_ERA5))+theme_bw()+
  ...
  annotate(geom = 'text',x = 6, y = -8,label=expression(R^2~"= 0.86"),size = 3)+
  annotate(geom = 'text',x = 6,y = -15,label=expression(Slope~"= 1.0"),size = 3)+
  theme(text = element_text(size = 8)) +
  labs(title = "TS3 vs ERA5 TS")

Tsoil_temp_test #great agreement

#df_avg

Tsoil_temp_test2= ggplot(data = df_avg,aes(TS_3_1_1,TS_ERA5))+theme_bw()+
  geom_hline(yintercept = 0,lty=2)+
  geom_vline(xintercept = 0,lty=2)+
  geom_point(alpha=0.2)+
  scale_fill_viridis_c()+
  ...
  annotate(geom = 'text',x = 6,y = -15,label=expression(Slope~"= 1.0"),size = 3)+
  theme(text = element_text(size = 8)) +
  labs(title = "TS3 vs ERA5 TS df_avg")

Tsoil_temp_test2 #great agreement


#dataset for TS comparison

TS_ERATS_comparison <-  df_avg %>%
  select(TS_3_1_1, TS_ERA5, year, DOY, season, date)


#below TS = 0 

TS_ERATS_below0 <- TS_ERATS_comparison %>%
  filter(TS_3_1_1 < 0, TS_ERA5 < 0)

summary(lm(TS_ERATS_below0 $TS_3_1_1 ~ TS_ERATS_below0 $TS_ERA5)) #R2 = 0.55; slope = 1.3

Tsoil_below0_test = ggplot(data = TS_ERATS_below0, aes(TS_3_1_1,TS_ERA5))+theme_bw()+
  geom_hline(yintercept = 0,lty=2)+
  geom_vline(xintercept = 0,lty=2)+
  geom_point(alpha=0.2)+
  scale_fill_viridis_c()+
  ...
  scale_x_continuous('Site TS3 (C)')+
  scale_y_continuous('ERA5 TS (C)')+
  annotate(geom = 'text',x = 6, y = -8,label=expression(R^2~"= 0.55"),size = 3)+
  annotate(geom = 'text',x = 6,y = -15,label=expression(Slope~"= 1.3"),size = 3)+
  theme(text = element_text(size = 8)) +
  labs(title = "below 0, TS3 vs ERA5 TS")

Tsoil_below0_test 

#above TS = 0 

TS_ERATS_above0 <- TS_ERATS_comparison %>%
  filter(TS_3_1_1 > 0, TS_ERA5 > 0)

summary(lm(TS_ERATS_above0 $TS_3_1_1 ~ TS_ERATS_above0 $TS_ERA5)) #R2 = 0.864; slope = 1.03

Tsoil_above0_test = ggplot(data = TS_ERATS_above0, aes(TS_3_1_1,TS_ERA5))+theme_bw()+
  geom_hline(yintercept = 0,lty=2)+
  geom_vline(xintercept = 0,lty=2)+
  geom_point(alpha=0.2)+
  scale_fill_viridis_c()+
  ...
  scale_y_continuous('ERA5 TS (C)')+
  annotate(geom = 'text',x = 6, y = -8,label=expression(R^2~"= 0.86"),size = 3)+
  annotate(geom = 'text',x = 6,y = -15,label=expression(Slope~"= 1.03"),size = 3)+
  theme(text = element_text(size = 8)) +
  labs(title = "below 0, TS3 vs ERA5 TS")
