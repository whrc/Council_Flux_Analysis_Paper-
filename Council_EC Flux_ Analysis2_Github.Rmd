...
#using WD modal because corr matrix can only use numeric variables, so can't use WD_cardinal 
env_flux_daily_notGF <- df_avg %>%
  select(FC, FCH4, GPP, RECO, TS_3_1_1, SWC_3_1_1, TA, WS, WD_modal, TA_ERA5, TS_ERA5) %>%
  rename(
    NEE = FC,
    CH4_flux = FCH4,
    GPP = GPP,
...
#### calc daily mean flux of CH4 for growing season for each year, then take 98% of that to get potential daily mean flux of winter, and multiply by # of days within the winter season for that year 
...
```
```{r}
adj_C_budgets <- read_xlsx(path ="C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/Seasonal_Cbudgets_Table_all_years_for_paper.xlsx", sheet = "R_template_adjCH4.2" )

# Reshape data for plotting
adj_C_budgets_long<- adj_C_budgets %>%
  pivot_longer(cols = c(Net_CO2, Net_CH4_adj, Total_Net_C, Net_CH4_adj_dailyflux),
               names_to = "flux_type",
               values_to = "flux")
