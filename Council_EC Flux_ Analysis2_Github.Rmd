...
####Flux v Environmental correlation matrix - gapfilled fluxes first & then non-gapfilled fluxes--> use non-GF for correlation matrix and stats* 
```{r}
...
#correlation matrix using non-gapfilled C fluxes ****** remember to use non-GF for all stats ***

# Environmental relationships
# Create a correlation plot between fluxes and environmental variables
env_flux_daily_notGF <- df_avg %>%
  select(FC, FCH4, TS_3_1_1, SWC_3_1_1, TA, WS, WD, TA_ERA5, TS_ERA5) %>%
  rename(
    CO2_flux = FC,
    CH4_flux = FCH4,
    Soil_temp = TS_3_1_1,
...
cor_matrix2 <- cor(env_flux_daily_notGF, use = "complete.obs")
...
ggplot(cor_long2, aes(x = Var1, y = Var2, fill = Correlation)) +
  ...
  theme(axis.text = element_text(size = 12, face = "bold"),
       plot.title = element_text(size = 14, face = "bold", hjust = 0.5)) +                 
  labs(title = "Correlation Matrix of not GF C Fluxes and Environmental Variables")
...
# option 1: Custom breaks at key values - can make figure look a bit clunky 
ggplot(cor_long2, aes(x = Var1, y = Var2, fill = Correlation)) +
  geom_tile() +
...
# #simple heatmap style correlation matrix visual --> meh 
# install.packages("lattice")
# print(cor_matrix2)
# 
# library(ggplot2)
# ggplot(data = cor_long2, aes(x=Var1, y=Var2,
#                                    fill=Correlation)) + 
# geom_tile()+
...
```
#Windrose - seasonal C fluxes by WD across all years
```{r}
