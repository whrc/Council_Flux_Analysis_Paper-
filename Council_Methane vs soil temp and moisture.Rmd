...
# 
# matches well with themoisture distribution:
# 
...
# Low R² values indicate other factors might be important; this isn't the best model 
...
#SWC is most important, but then followed closely by tsoil, then TS:SWC interaction --> comparatively, wd was usually 1 or 2, it has not dropped to 4th most important 
#Slope = 0.566; R2 = 0.599 (best fit per R2 yet)
...
#Optimal number of trees for NEE? Just to see? 
####OOB for NEE
```{r}
set.seed(123)
...
# Better slope, though still underestimating
# 
# Variable Importance:
...
```{r}
library(mgcv)
gam_model <- gam(FCH4 ~ s(tsoil) + s(swc) + ti(tsoil, swc), data = train.ch4)
```
...
library(lme4)
library(e1071)
svr_model <- svm(FCH4 ~ tsoil + swc, data = train.ch4)
```
...
# SVR Model - Support Vector Regression (SVR)
...
#svr_pred <- predict(svr_model, newdata = test.ch4)
...
#trying to get SVR to work 
```{r}
# Create a simple numbered index
test.ch4$row_id <- 1:nrow(test.ch4)

# Fit model and get predictions
svr_model_new <- svm(fch4 ~ tsoil + swc, data = test.ch4)
svr_pred_new <- predict(svr_model_new, newdata = test.ch4)

# Create dataframe with predictions and actual values
pred_df <- data.frame(
    row_id = test.ch4$row_id,
    actual = test.ch4$fch4,
    predicted = svr_pred_new
)

# Check which rows are missing predictions
print("Rows with predictions:")
print(length(svr_pred_new))
print("Range of row IDs with predictions:")
print(range(which(!is.na(svr_pred_new))))

# Look at some summary statistics of the data
print("Summary of predictors:")
print(summary(test.ch4[c("tsoil", "swc", "fch4")]))
```
