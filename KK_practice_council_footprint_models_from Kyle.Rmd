...
get_credit("Esri.WorldImagery") #for credit/citations
...
gs_final <- ggplot() +
  ...
  #White contour lines for contribution area 30, 60, 90%
      geom_contour(data = df_cumulative,
               aes(x = x, y = y, z = cumsum_pct, color = after_stat(level)),
               breaks = c(30, 60, 90),
               #color = "white",
               #color = "cumsum_pct",
               linewidth = 1) +
...
# Add North arrow
  ...
  line_col = "black",
   text_size = 12,         # Text size (default is ~10)
    ...
    text_col = "black"),
  location = "tr",      # top-left (or "tr", "bl", "br")
...
#transform the coordinates to raster files - fall
```{r}
#check the coord ref system of your image and the fp dataframe and make sure they match* 
...
# Plot each one to see which is which
ggdraw(legend[[1]])
ggdraw(legend[[2]])
...
# Remove legends from growing season and fall
gs_noleg <- gs_final + theme(legend.position = "none")
fall_noleg <- fall_final + theme(legend.position = "none")

# Keep legend on winter, set to bottom
winter_leg <- winter_final + theme(
...
final_combined <- (gs_noleg | fall_noleg | winter_leg) +
...
```{r}
#With footprint contour shading 
# ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/footprint_combined_seasonal_footprintarea.png", final_combined, 
#         width = 8, height = 4.3, dpi = 600)

#Without contour shading, only contour lines 
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/footprint_combined_seasonal_footprintarea2.png", final_combined, 
        width = 8, height = 4.3, dpi = 600)
```
...
#ALL - footprint - total average 


```{r}
library(FREddyPro)

fp_all = Average(#fetch = 250,         #can add a custom fetch or leave blank  - takes avg of entire dataset, can take footprint to hour or an avg you determine 
             height = 2.650,         #height of tower / measurement height 
             grid = 100,           #size of grid cells in footprint (resolution) (100 x 100)
             speed = df$WS,        #wind speed
             direction = df$WD,    #wind direction
             uStar = df$USTAR,     #ustar
             zol = df$ZL,          #monin okubov length  (air upward movement with temp )
             sigmaV = df$V_SIGMA)  #standard deviation of V wind direction

#see materials on diff models kyle sent - flat sfc without big landscape differences used here but there are other model types 

#calculate set percentage of footprint - freddypro package * 
#calculates the area contributing 80% of measured flux; 80% contour line 
percent.fp_all = calculatePercentFootprint(ftp = fp_all,percent = 80)  
...
fpdf_all = exportFootprintPoints(ftp = fp_all,xcoord = spatial.tower@coords[1] ,ycoord = spatial.tower@coords[2] )

percent.df_all = exportFootprintPoints(ftp = percent.fp_all,xcoord = spatial.tower@coords[1], ycoord = spatial.tower@coords[2])

#Shows probability of fluxes coming from a larger region; purple = basically outside of the 80% footprint 
ggplot(data = fpdf_all)+
  geom_raster(aes(x,y,fill = z*100))+ #z*100 converts probability to percentage 
  scale_fill_viridis_c()+ #purple = very low probability, yellow = high probability 
  theme(legend.position = "bottom")

#Shows the 80% footprint, 
ggplot(data = percent.df_all)+
  geom_raster(aes(x,y,fill = z),interpolate = T)+ #interpolate = T smooths out 
  scale_fill_viridis_c(na.value = "transparent",direction = -1)+ #reverses colors but this shows that purple = outside edge of 80% footprint probability -- contribution boundary at 80% and represents the "effective fetch" (important for relating landscape features to fluxes*)
  theme(legend.position = "bottom")


 sum(fpdf_all$z) #--> double check probability (z) amounts to ~1
...
#transform the coordinates to raster files - all 
```{r}
#check the coord ref system of your image and the fp dataframe and make sure they match* 
...
#print coord ref system from the % footprint dataframe --> in UTM, so need to convert to make them the same
print(head(percent.df_all))

# Use UTM CRS (because percent.df_all has UTM coordinates):
utm_crs <- CRS("+init=epsg:32603")
r_all = rasterFromXYZ(xyz = percent.df_all, crs = utm_crs) 

#r = rasterFromXYZ(xyz = percent.df_all,crs = st) #converts footprint dataframe to raster obj, uses coord ref system from "st" which references the spatial tower image 
#r2 = projectRaster(from = r,crs = st) #assigns coord reference system (from image) -- **MAKE SURE it's in the coord system you want. For Maptiles, crs is in lat/long already, not UTM

r2_all = projectRaster(from = r_all,crs = utm_crs) 

df2_all = as.data.frame(r2_all,xy=T)
...
rll_all = projectRaster(from = r_all,crs = latlon) #change % to lat/long so you can see the majority of the footprint over sat imagery

#check 
 print(crs(rll_all)) #lat/long
 print(extent(rll_all)) #lat/long
 
df_latlon_all <- as.data.frame(rll_all, xy = TRUE)
 head(df_latlon_all)
summary(df_latlon_all$x)  # Should be around -163.7
summary(df_latlon_all$y)  # Should be around 64.86
#check plot is in lat/long 
names(df_latlon_all) #x, y, z where z is probability 
plot(rll_all, main = "Footprint in Lat/Lon")
...
plot1_all <- ggplot() +
  ...
  # Footprint overlay (replaces layer_spatial(rll))
  geom_raster(data = df_latlon_all,
              aes(x = x, y = y, fill = z),  # Use "layer" column
              alpha = 0.6) +
...
plot3_all <- ggplot() +
  ...
  # Contour lines (replaces geom_contour with percent.df)
  geom_contour(data = df_latlon_all,
               aes(x = x, y = y, z = z, color = after_stat(level)),
               bins = 10,  # Adjust number of contour lines
               linewidth = 1) +
...
```{r}
# Create raster from 80% contour
r_80_all <- rasterFromXYZ(xyz = percent.df_all, crs = utm_crs)

# Project to lat/lon
rll_80_all <- projectRaster(from = r_80_all, crs = latlon)

# Convert to data frame
df_80_latlon_all <- as.data.frame(rll_80_all, xy = TRUE)
...
plot4_all <- ggplot() +
  ...
  # Or as filled contour
  geom_contour_filled(data = df_80_latlon_all,
                     aes(x = x, y = y, z = z)) +
...
# Calculate cumulative contribution
df_cumulative_all <- df_latlon_all %>%
  filter(!is.na(z)) %>%
  arrange(desc(z)) %>%  # Sort by highest contribution first
  mutate(
    cumsum_raw = cumsum(z),
...
# Check
summary(df_cumulative_all$cumsum_pct)  # Should go from ~0 to 100

# ============================================================================
# PLOT LIKE THE EXAMPLE
# ============================================================================

plot_cumulative_all <- ggplot() +
  # Satellite basemap
  geom_spatraster_rgb(data = image_400m, maxcell = 5e5) +
  ...
  # Color gradient showing per-cell contribution (semi-transparent)
  geom_raster(data = df_cumulative_all,
              aes(x = x, y = y, fill = z),
              alpha = 0.5) +
  scale_fill_viridis_c(
    name = "% Contribution",
  ...
  # WHITE CONTOUR LINES showing cumulative contribution
  geom_contour(data = df_cumulative_all,
               aes(x = x, y = y, z = cumsum_pct),
               breaks = c(30, 60, 90),  # cumulative
               #color = "white",
               linewidth = 0.5) +
...
#Contour lines 
  ...
  #coord_sf() +  #positions things around the figure on the outside border; padding around image
  coord_sf(expand = FALSE) +  # Remove padding around plot, places scale bar and compass within fig
 # labs(title = "Mean EC Footprint - all") +
  labs(title = "Mean Footprint for Study Period") +
  theme_minimal() +
  theme(   #removes the gridlines behind the image 
      panel.grid = element_blank(),
  panel.background = element_blank(),
...
```{r}
#with footprint shading 
# ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/footprint_mean_total_footprintarea.png", all_final, 
#         width = 8, height = 4.3, dpi = 600)

#no shading, only contour lines 
ggsave("C:/Users/kkent/Documents/Github Flux Network/Council_Flux_Analysis_Paper/Council Figures/footprint_mean_total_footprintarea2.png", all_final, 
        width = 8, height = 4.3, dpi = 600)
...
#Identify high methane dates to check outside of Nov2019


```{r}
df_highCH4 <- df_avg %>%
    filter(TS_3_gapfilled < 0.5 ) 

df_highCH4$FCH4_umol <- df_highCH4$FCH4/1000


df_highCH4_dates <- df_highCH4 %>%
    filter(FCH4_umol > 0.01 ) 
